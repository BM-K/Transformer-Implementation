...
class MultiheadAttention(nn.Module):
    def __init__(self):
        ...
        self.W_Q = nn.Linear(d_model, d_k * n_heads)  # init (512 x 64 * 8)
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs):  # enc_inputs : [batch_size x source_len]
        ...
        enc_self_attn_mask = get_attn_pad_mask(enc_inputs, enc_inputs)  # PAD 에 MASK
        enc_self_attns = []
        for layer in self.layers:
            enc_outputs, enc_self_attn = layer(enc_outputs, enc_self_attn_mask)
            #  enc_outputs = [batch x len_q x d_model]
            #  enc_self_attn_mask = [batch x len_q x len_k]
...
def showgraph(attn, num):
    ...
    plt.show()
    plt.savefig(f'fig{num}.png', dpi=300)
...
showgraph(enc_self_attns, number)
number+=1

print('first head of last state dec_self_attns')
showgraph(dec_self_attns, number)
number+=1

print('first head of last state dec_enc_attns')
showgraph(dec_enc_attns, number)